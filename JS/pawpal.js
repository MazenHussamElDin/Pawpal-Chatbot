...
class Bot {
  ...
  async processMessage(userId, msg) {
    ...
    if (questionSplits.length > 1) {
      ...
      for (const partial of questionSplits) {
        const tempSession = JSON.parse(JSON.stringify(session)); // isolate state
        const answer = await this.processMessage(userId, partial);
        if (answer && !results.includes(answer)) {
          results.push(`🟡 Frage: "${partial}" → ${answer}`);
        }

        // Reset flags between subquestions
  ...
  // === SYMPTOM DETECTION ===
  const directSymptom = allSymptoms.find(symptom => loweredMsg.includes(symptom));
  if (directSymptom) {
    const serious = this.symptomData.seriousSymptoms.includes(directSymptom);
    return serious
      ? `${directSymptom.charAt(0).toUpperCase() + directSymptom.slice(1)} klingt ernst. 🏥 Geh bitte schnell zur Tierarztpraxis!`
      : `${directSymptom.charAt(0).toUpperCase() + directSymptom.slice(1)} ist meist harmlos. Ruhige Umgebung, Wasser bereitstellen, und beobachten. 🐾`;
  }
